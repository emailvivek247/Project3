...
************************************************************************************************************/
	...
	function moveSingleElement()
	{
		...
		for(var no=0;no<tmpFromBox.options.length;no++){                         
			...
			if(tmpFromBox.options[no].selected){
				...
				tmpToBox.options[tmpToBox.options.length] = new Option(tmpFromBox.options[no].text,tmpFromBox.options[no].value);
                                 if (currentselection == ""){
                                     currentselection = tmpFromBox.options[no].text;                                  
                                 }
                                 else {
                                     currentselection = currentselection + "," + tmpFromBox.options[no].text;                                  
                                 }
		...
		for(var no=0;no<tmpTextArray2.length;no++){
			...
			tmpToBox.options[no] = new Option(items[0],items[1]);
                 ...
                 if (action == "ADD" && currentselection != ""){compareDoctypes(currentselection);}else if(action == "REMOVE" && currentselection != ""){removeDoctype(currentselection);}
	}
	...
	function createMovableOptions(fromBox,toBox,totalWidth,totalHeight,labelLeft,labelRight)
	{
		...
		subDiv.style.width = (Math.floor(totalWidth/2) - 20) + 'px';
		fromObj.style.width = (Math.floor(totalWidth/2) - 20) + 'px';
		...
		var subDiv = document.createElement('DIV');
		subDiv.style.width = (Math.floor(totalWidth/2) - 20) + 'px';
		toObj.style.width = (Math.floor(totalWidth/2) - 20) + 'px';
