...
public class SDLSimpleUrlAuthenticationFailureHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
        ...
        logger.debug("Exception Occured while Authenticating");
        if (exception instanceof UsernameNotFoundException) {
            errorMessage = messageSource.getMessage("security.authentication.usernamenotfound", null, Locale.US);
        } else if (exception instanceof BadCredentialsException) {
            errorMessage = messageSource.getMessage("security.authentication.badcredentials", null, Locale.US);
        ...
        request.getSession().setAttribute(AUTHENTICATION_ERROR_MESSAGE, errorMessage);
