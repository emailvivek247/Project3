...
VALUES ('VABEACHCMS', 'City of Virginia Beach', 'Y', 'Virginia Beach', 'VA', 'Y', 'America/New_York', @NODE_ID, GETDATE(), GETDATE(), 'SYSTEM', 
...
DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'VABEACHCMS')
...
VALUES (@SITE_ID, 'Records Management/VABEACHCMS/', 'noReply@amcad.com', 'Payment Confirmation - Roam Virginia Beach CMS Records Management', 
	'Subscription Change Notification - Roam Virginia Beach CMS Records Management', 'Cancelled Subscription Notification - Roam Virginia Beach CMS Records Management', 
	'Subscription Reactivated - Roam Virginia Beach CMS Records Management', 'Recurring Payment Posted Successfully - Roam Virginia Beach CMS Records Management', 
	'Recurring Payment Failed - Roam Virginia Beach CMS Records Management',
	...
	'recurringPaymentUnsuccessfulTemplate.stl', GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'Web Purchase Confirmation - Roam Virginia Beach CMS Records Management', 
	'webPaymentConfirmation.stl', 'Subscription Removed - Roam Virginia Beach CMS Records Management','removeSubscription.stl', 'SYSTEM', 
	'Access Authorization - Roam Virginia Beach CMS Records Management', 'accessAuthorization.stl', 'Pay As You Go Purchase Confirmation - Roam Virginia Beach CMS Records Management'
	,'payAsYouGoPaymentConfirmation.stl','Subscription Add Notification - Roam Virginia Beach CMS Records Management')
GO
...
VALUES ('VABEACHCMS_ADMIN', 'Virginia Beach CMS ROAM ADMIN', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'ACCESS TO THE ROAM DESKTOP.', 'N', NULL, 'SYSTEM', 'N', 'N', 1, 'N', 0)
INSERT INTO AUTH_ACCESS (ACCESS_CD, ACCESS_DESCR, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, GUEST_FLG, ACTIVE, ACCESS_FEATURES, DEFAULT_ACCESS_FLG, ACCESS_ORDER, CREATED_BY, IS_AUTHORIZATION_REQUIRED, IS_VISIBLE, MAX_USERS_ALLOWED, IS_FIRM_LEVEL_ACCESS, MAX_DOCUMENTS_ALLOWED)
VALUES ('VABEACHCMS_IMAGE_ACCESS', 'Virginia Beach Unlimited Image Access', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'Access to Virginia Beach Land Record along with Image access. The subscription fee is $50 per month until cancelled, but non refundable for the paid subscription period.', 'Y', '1', 'SYSTEM', 'Y', 'Y',1, 'N', 0)
GO

--- INSERT SITE ACCESS INFORMATION ---
DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'VABEACHCMS')
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHCMS_ADMIN')
...
SET @ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHCMS_IMAGE_ACCESS')
INSERT INTO ECOMM_SITE_ACCESS (SITE_ID, ACCESS_ID, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, CREATED_BY)
VALUES (@SITE_ID, @ACCESS_ID, GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')
GO
...
DECLARE @SITEACCESS_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE_ACCESS WHERE ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHCMS_IMAGE_ACCESS'))
...
--- INSERT PROFIT SHARE ---
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHCMS_IMAGE_ACCESS')
