...
DELETE FROM ECOMM_NON_RECURRING_FEE WHERE ID IN (
SELECT NRF.ID FROM AUTH_ACCESS AA, ECOMM_SITE_ACCESS ESA, ECOMM_SITE ES, ECOMM_NON_RECURRING_FEE NRF 
WHERE ES.ID = ESA.SITE_ID AND ESA.ACCESS_ID = AA.ID AND NRF.SITEACCESS_ID = ESA.ID AND ES.NAME = 'SMITH' AND NRF.FEE_UNDER_PAGE_THRESHOLD = '0.00'
)
GO


UPDATE ECOMM_NON_RECURRING_FEE SET FEE_UNDER_PAGE_THRESHOLD = '0.50', PAGE_THRESHOLD = '1', FEE_OVER_PAGE_THRESHOLD = '0.50' 
WHERE ID IN (SELECT NRF.ID  FROM AUTH_ACCESS AA, ECOMM_SITE_ACCESS ESA, ECOMM_SITE ES, ECOMM_NON_RECURRING_FEE NRF 
WHERE ES.ID = ESA.SITE_ID AND ESA.ACCESS_ID = AA.ID AND NRF.SITEACCESS_ID = ESA.ID AND ES.NAME = 'SMITH' AND IS_SERVICE_FEE = 'Y')
GO

UPDATE ECOMM_NON_RECURRING_FEE SET FEE_UNDER_PAGE_THRESHOLD = '0.50', PAGE_THRESHOLD = '1', FEE_OVER_PAGE_THRESHOLD = '0.50' 
WHERE ID IN (SELECT NRF.ID  FROM AUTH_ACCESS AA, ECOMM_SITE_ACCESS ESA, ECOMM_SITE ES, ECOMM_NON_RECURRING_FEE NRF 
WHERE ES.ID = ESA.SITE_ID AND ESA.ACCESS_ID = AA.ID AND NRF.SITEACCESS_ID = ESA.ID AND ES.NAME = 'SMITH' AND IS_SERVICE_FEE = 'N')
GO

UPDATE AUTH_ACCESS SET ACCESS_DESCR = 'Pay as You Go ($1.00 a page) with Image Preview (free)',  ACCESS_FEATURES = '<b>Subscription cost:</b> $1.00 Per Page Printed<br /><b>Access to Collin County Official Public Records</b><br /><b>Data Access:</b> Index Access wtih free Image Preview (with Watermark) and ability to purchase images without watermarks for printing<br /><b>Additional Features Include:</b> Full Field Text Searching, Faceting, Smart Navigation, User Defined Search Sorting, Shopping Cart Functionality, Up to 100 records Per Page'
WHERE ACCESS_CD = 'SMITH_PERPAGE_ACCESS'
GO

UPDATE AUTH_ACCESS SET ACCESS_FEATURES = '<b>Subscription Cost: </b>$100<br /><b>Unlimited searches<br />Available Records:</b>1963 - Present<br /><b>Full Document Preview w/ Watermark<br />Unlimited Document Saving and Printing<br />$1 Per Page to Print<br />Max Results Per Page:</b> 100<br /><b>Additional Features:</b> Full Field Text Searching, Faceting, Smart Navigation, User Defined Search Sorting, Shopping Cart Functionality'
WHERE ACCESS_CD = 'SMITH_UNOFFICIAL_1'
GO
