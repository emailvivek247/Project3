...
VALUES ('SUFFOLK', 'City of Suffolk', 'Y', 'Suffolk', 'VA', 'Y', 'America/New_York', @NODE_ID, GETDATE(), GETDATE(), 'SYSTEM', 
	'You can choose only one of the available subscriptions for Suffolk.', 'Y', 'Y', 'Suffolk', 'SYSTEM', 72, 30, 30)
GO

--- INSERT SITE CONFIGURATION INFORMATION ---
DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'SUFFOLK')
...
VALUES (@SITE_ID, 'Records Management/SUFFOLK/', 'noReply@amcad.com', 'Payment Confirmation - Roam Suffolk Records Management', 
	'Subscription Change Notification - Roam Suffolk Records Management', 'Cancelled Subscription Notification - Roam Suffolk Records Management', 
	'Subscription Reactivated - Roam Suffolk Records Management', 'Recurring Payment Posted Successfully - Roam Suffolk Records Management', 
	'Recurring Payment Failed - Roam Suffolk Records Management',
	...
	'recurringPaymentUnsuccessfulTemplate.stl', GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'Web Purchase Confirmation - Roam Suffolk Records Management', 
	'webPaymentConfirmation.stl', 'Subscription Removed - Roam Suffolk Records Management','removeSubscription.stl', 'SYSTEM', 
	'Access Authorization - Roam Suffolk Records Management', 'accessAuthorization.stl')
GO
...
VALUES ('SUFFOLK_ADMIN', 'Suffolk ROAM ADMIN', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'ACCESS TO THE ROAM DESKTOP.', 'N', NULL, 'SYSTEM', 'N')
INSERT INTO AUTH_ACCESS (ACCESS_CD, ACCESS_DESCR, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, GUEST_FLG, ACTIVE, ACCESS_FEATURES, DEFAULT_ACCESS_FLG, ACCESS_ORDER, CREATED_BY, IS_AUTHORIZATION_REQUIRED)
VALUES ('SUFFOLK_IMAGE_ACCESS', 'Suffolk Premier Subscription', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'Access Suffolk County Land Records. The subscription fee is $50 per month until cancelled, but non refundable for the paid subscription period.', 'Y', '1', 'SYSTEM', 'Y')
INSERT INTO AUTH_ACCESS (ACCESS_CD, ACCESS_DESCR, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, GUEST_FLG, ACTIVE, ACCESS_FEATURES, DEFAULT_ACCESS_FLG, ACCESS_ORDER, CREATED_BY, IS_AUTHORIZATION_REQUIRED)
VALUES ('SUFFOLK_FREE_ACCESS', 'Suffolk Free Access', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'Access to Suffolk County Land Records. This does not include access to the images. Please choose the Premium subscription to get access to the images.', 'N', '2', 'SYSTEM', 'N')
GO


--- INSERT SITE ACCESS INFORMATION ---
DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'SUFFOLK')
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'SUFFOLK_ADMIN')
...
SET @ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'SUFFOLK_IMAGE_ACCESS')
INSERT INTO ECOMM_SITE_ACCESS (SITE_ID, ACCESS_ID, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, CREATED_BY)
VALUES (@SITE_ID, @ACCESS_ID, GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')
SET @ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'SUFFOLK_FREE_ACCESS')
INSERT INTO ECOMM_SITE_ACCESS (SITE_ID, ACCESS_ID, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, CREATED_BY)
VALUES (@SITE_ID, @ACCESS_ID, GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')
GO
...
DECLARE @SITEACCESS_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE_ACCESS WHERE ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'SUFFOLK_IMAGE_ACCESS'))
...
SET @SITEACCESS_ID = (SELECT TOP 1 ID FROM ECOMM_SITE_ACCESS WHERE ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'SUFFOLK_FREE_ACCESS'))
INSERT INTO ECOMM_SUBSCRIPTIONFEE (SUBSCRIPTION_TYP_ID, SITEACCESS_ID, FEE, TERM, CURRENCY, ACTIVE, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, CREATED_BY)
...
INSERT INTO ECOMM_BANK_DETAILS (SITE_ID, FROM_FNAME, FROM_LNAME, FROM_MINITIAL, FROM_ADDRLINE1, FROM_ADDRLINE2, FROM_CITY, FROM_STATE, FROM_ZIPCODE, FROM_PHONENUM,
						...
						VALUES (@SITE_ID, 'AMCAD', '', '','15867 N Mountain Road','','Broadway','VA','22815','','BB&T','111-20/121','056005318','0005132069613', 10000, 10000, 19999,
...
INSERT INTO AUTH_USERS (EMAIL_ID, PASSWORD, FIRST_NAME, LAST_NAME, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ADDRESS_1, ADDRESS_2, CITY, STATE, ZIP, PHONE, CREATED_IP, 
				...
				VALUES ('suffolkadmin@roam.com', '2851FFD2E30E91904A9E86F326E442065D75DB65A028854E9A5ACEE67E53EA59', 'ROAM', 'ADMIN', GETDATE(), GETDATE(), 'Admin', null, null, null, null, null, null, '1.1.1.1.1', 
						'Y', 'Y', 'Y', 'Y', GETDATE(), 'RECORDSMANAGEMENT', GETDATE(), 'SYSTEM')
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'SUFFOLK_ADMIN')
DECLARE @USER_ID AS INT  = (SELECT TOP 1 ID FROM AUTH_USERS WHERE EMAIL_ID = 'suffolkadmin@roam.com')
...
VALUES (@SITE_ID, 'City of Suffolk Clerks Office', '150 N Main St', '', 'Suffolk', 'VA','23434','7575147812','','','OTC',GETDATE(), GETDATE(), 'SYSTEM','Y', 'SYSTEM')
GO

--- INSERT PROFIT SHARE ---
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'SUFFOLK_IMAGE_ACCESS')
