...
INSERT INTO ECOMM_SITE (NAME, DESCRIPTION, ACTIVE, COUNTY, STATE, AUTOACTIVATE, TIMEZONE, NODE_ID, DATE_TIME_CREATED, DATE_TIME_MOD, 
	...
	ACH_HOLD_PERIOD, IS_FIRM_NUMBER_REQUIRED,	IS_BAR_NUMBER_REQUIRED,	IS_FREE_SITE)
VALUES ('VABEACHGOV', 'City of Virginia Beach Gov', 'Y', 'Virginia Beach', 'VA', 'Y', 'America/New_York', @NODE_ID, GETDATE(), GETDATE(), 'SYSTEM', 
	'You can choose only one of the available subscriptions of a particular type for Virginia Beach.', 'Y', 'Y', 'Virginia Beach', 'SYSTEM', 3, 30, 30, 3, 'N', 'N', 'N')
GO

--UPDATE SITE_CONFIGURATION SET EMAIL_TEMPLATE_FOLDER = 'CaseManagement/VABEACHGOV/' WHERE CANCEL_SUBSCRIPTION_SUB = 'Cancelled Subscription Notification - Roam Virginia Beach CMS Case Management'


--DELETE FROM SITE_CONFIGURATION WHERE SITE_ID = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'VABEACHGOV')
--- INSERT SITE CONFIGURATION INFORMATION ---
DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'VABEACHGOV')
INSERT INTO SITE_CONFIGURATION (SITE_ID, EMAIL_TEMPLATE_FOLDER, FROM_EMAIL_ADDRESS, PAYMENT_CONF_SUB, CHANGE_SUBSCRIPTION_SUB, CANCEL_SUBSCRIPTION_SUB, 
	...
	REMOVE_SUBSCRIPTION_TEMPLATE, CREATED_BY, ACCESS_AUTHORIZATION_SUB, ACCESS_AUTHORIZATION_TEMPLATE, PAYASUGO_PAYMENT_CONFIRMATION_SUB,	
	PAYASUGO_PAYMENT_CONFIRMATION_TEMPLATE,	ADD_SUBSCRIPTION_SUB)
VALUES (@SITE_ID, 'CaseManagement/vabeachgov/', 'noReply@amcad.com', 'Payment Confirmation - Roam Virginia Beach Case Management', 
	'Subscription Change Notification - Roam Virginia Beach Case Management', 'Cancelled Subscription Notification - Roam Virginia Beach Case Management', 
	'Subscription Reactivated - Roam Virginia Beach Case Management', 'Recurring Payment Posted Successfully - Roam Virginia Beach Case Management', 
	'Recurring Payment Failed - Roam Virginia Beach Case Management',
	...
	'recurringPaymentUnsuccessfulTemplate.stl', GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'Web Purchase Confirmation - Roam Virginia Beach Case Management', 
	'webPaymentConfirmation.stl', 'Subscription Removed - Roam Virginia Beach Case Management','removeSubscription.stl', 'SYSTEM', 
	'Access Authorization - Roam Virginia Beach Case Management', 'accessAuthorization.stl', 'Pay As You Go Purchase Confirmation - Roam Virginia Beach Case Management'
	,'payAsYouGoPaymentConfirmation.stl','Subscription Add Notification - Roam Virginia Beach Case Management')
GO
...
INSERT INTO AUTH_ACCESS (ACCESS_CD, ACCESS_DESCR, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, GUEST_FLG, ACTIVE, ACCESS_FEATURES, DEFAULT_ACCESS_FLG, ACCESS_ORDER, CREATED_BY, IS_AUTHORIZATION_REQUIRED, IS_VISIBLE, MAX_USERS_ALLOWED, IS_FIRM_LEVEL_ACCESS, MAX_DOCUMENTS_ALLOWED)
VALUES ('VABEACHGOV_ADMIN', 'Virginia Beach CMS ROAM ADMIN', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'ACCESS TO THE ROAM DESKTOP.', 'N', NULL, 'SYSTEM', 'N', 'N', 1, 'N', 0)
INSERT INTO AUTH_ACCESS (ACCESS_CD, ACCESS_DESCR, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, GUEST_FLG, ACTIVE, ACCESS_FEATURES, DEFAULT_ACCESS_FLG, ACCESS_ORDER, CREATED_BY, IS_AUTHORIZATION_REQUIRED, IS_VISIBLE, MAX_USERS_ALLOWED, IS_FIRM_LEVEL_ACCESS, MAX_DOCUMENTS_ALLOWED)
VALUES ('VABEACHGOV_IMAGE_ACCESS', 'Virginia Beach Unlimited Image Access', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'Access to Virginia Beach Land Record along with Image access. The subscription fee is $75 per month until cancelled, but non refundable for the paid subscription period.', 'Y', '1', 'SYSTEM', 'Y', 'Y',1, 'N', 0)
GO


--- INSERT SITE ACCESS INFORMATION ---
DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'VABEACHGOV')
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHGOV_ADMIN')
...
SET @ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHGOV_IMAGE_ACCESS')
INSERT INTO ECOMM_SITE_ACCESS (SITE_ID, ACCESS_ID, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, CREATED_BY)
VALUES (@SITE_ID, @ACCESS_ID, GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')
GO
...
DECLARE @SITEACCESS_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE_ACCESS WHERE ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHGOV_IMAGE_ACCESS'))
...
VALUES (@SUBSCRIPTION_TYP_ID, @SITEACCESS_ID, '75.00', 0, 'USD', 'Y', GETDATE(), GETDATE(), 'SYSTEM', 'SYSTEM')
...
INSERT INTO ECOMM_BANK_DETAILS (SITE_ID, FROM_FNAME, FROM_LNAME, FROM_MINITIAL, FROM_ADDRLINE1, FROM_ADDRLINE2, FROM_CITY, FROM_STATE, FROM_ZIPCODE, FROM_PHONENUM,
						...
						VALUES (@SITE_ID, 'AMCAD', '', '','15867 N Mountain Road','','Broadway','VA','22815','','BB&T','111-20/121','056005318',						
						'0005132069613', @HIGHESTSTARTCHECKNUM, @HIGHESTSTARTCHECKNUM, @HIGHESTENDCHECKNUM,
...
INSERT INTO AUTH_USERS (EMAIL_ID, PASSWORD, FIRST_NAME, LAST_NAME, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ADDRESS_1, ADDRESS_2, CITY, STATE, ZIP, PHONE, CREATED_IP, 
				...
				VALUES ('vbeachcmsadmin@roam.com', '2851FFD2E30E91904A9E86F326E442065D75DB65A028854E9A5ACEE67E53EA59', 'ROAM', 'ADMIN', GETDATE(), GETDATE(), 'Admin', null, null, null, null, null, null, '1.1.1.1.1', 
						'Y', 'Y', 'Y', 'Y', GETDATE(), 'RECORDSMANAGEMENT', GETDATE(), 'SYSTEM')
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHGOV_ADMIN')
DECLARE @USER_ID AS INT  = (SELECT TOP 1 ID FROM AUTH_USERS WHERE EMAIL_ID = 'vbeachcmsadmin@roam.com')
INSERT INTO AUTH_USERS_ACCESS (ACCESS_ID, USER_ID, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, COMMENTS, IS_ACCESS_OVERRIDDEN, CREATED_BY, IS_AUTHORIZED, IS_FIRM_ACCESS_ADMIN, FIRM_ADMIN_USER_ACCESS_ID)
				VALUES (@ACCESS_ID, @USER_ID, GETDATE(), GETDATE(), 'SYSTEM', 'Y', NULL, 'N', 'SYSTEM', 'Y', 'N', NULL)
GO
...
--- INSERT PROFIT SHARE ---
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'VABEACHGOV_IMAGE_ACCESS')
INSERT INTO ECOMM_PROFIT_SHARE (ACCESS_ID, CLIENT_SHARE, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, CREATED_BY)
            VALUES(@ACCESS_ID, 0.35, GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')								
