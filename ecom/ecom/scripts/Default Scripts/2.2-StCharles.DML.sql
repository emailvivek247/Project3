...
USER_RETENTION_DAYS, IS_FIRM_NUMBER_REQUIRED, IS_BAR_NUMBER_REQUIRED, IS_FREE_SITE)
	VALUES ('STCHARLES', 'St. Charles County', 'Y', '', 'MO', 'Y', 'America/Chicago', @NODE_ID, GETDATE(), GETDATE(), 'SYSTEM',
	'You can choose only one of the available subscriptions', 'Y', 'Y', 'St. Charles County', 'SYSTEM', 3, 30, 30, 'Y', 'N', 'N')
GO
...
REMOVE_SUBSCRIPTION_TEMPLATE, CREATED_BY, ACCESS_AUTHORIZATION_SUB, ACCESS_AUTHORIZATION_TEMPLATE, PAYASUGO_PAYMENT_CONFIRMATION_SUB, PAYASUGO_PAYMENT_CONFIRMATION_TEMPLATE, ADD_SUBSCRIPTION_SUB)
	VALUES (@SITE_ID, 'RecordsManagement/StCharles/', 'noReplyTest@amcad.com', 'Payment Confirmation - Roam St. Charles Records Management', 'Subscription Change Notification - Roam St. Charles Records Management',
	'Cancelled Subscription Notification - Roam St. Charles Records Management', 'Subscription Reactivated - Roam St. Charles Records Management', 'Recurring Payment Posted Successfully - Roam St. Charles Records Management',
	'Recurring Payment Failed - Roam St. Charles Records Management', 'paymentConfirmation.stl', 'changeConfirmation.stl', 'cancelConfirmation.stl', 'cancelConfirmation.stl', 'recurringPaymentSuccessfulTemplate.stl',
	'recurringPaymentUnsuccessfulTemplate.stl', GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'Web Purchase Confirmation - Roam St. Charles Records Management', 'webPaymentConfirmation.stl', 'Subscription Removed - Roam St. Charles Records Management',
	'removeSubscription.stl', 'SYSTEM', 'Access Authorization - Roam St. Charles Records Management', 'accessAuthorization.stl', 'Pay As You Go Purchase Confirmation - ROAM St. Charles Case Management', 'payAsYouGoPaymentConfirmation.stl',
	'Subscription Add Notification - St. Charles Records Management')
GO
...
INSERT INTO AUTH_TERMS (TERM_DESC, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, SITE_ID, TERM_TYP_ID, ACTIVE, CREATED_BY)
	VALUES (
	'<b>Terms and Conditions</b><br/><br/><ol><li><u>Access to and use of the eAccess website (hereinafter "Services") provided for the St. Charles Supreme
	...
	by AMCAD. For the purposes of this User Agreement, the St. Charles Supreme Court shall be referred to as the "Court," and you, the User, will be referred
...
IS_FIRM_LEVEL_ACCESS, MAX_USERS_ALLOWED, MAX_DOCUMENTS_ALLOWED)
	VALUES ('STCHARLES_PERPAGE_ACCESS', 'Pay as You Go ($1.00 For First Page; $0.50 Per Page For Each Page After) with Image Preview (Free)', GETDATE(),
	GETDATE(), 'SYSTEM', 'N', 'Y', 'Provides access to St. Charles Court Case Records. This subscription is Pay-As-You- GO and documents can be purchased individually. The cost is $10 for each document.', 'N', '1', 'SYSTEM', 'N', 'N', 0, 0)
GO
INSERT INTO AUTH_ACCESS (ACCESS_CD, ACCESS_DESCR, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, GUEST_FLG, ACTIVE, ACCESS_FEATURES, DEFAULT_ACCESS_FLG, ACCESS_ORDER, CREATED_BY, IS_AUTHORIZATION_REQUIRED, IS_FIRM_LEVEL_ACCESS,
MAX_USERS_ALLOWED, MAX_DOCUMENTS_ALLOWED)
	VALUES ('STCHARLES_ALL_ACCESS', 'Monthly Unlimited Official', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y',
	'Provides access to St. Charles Court Case Records. The subscription fee is $80 per month with a limit of 20 documents per month until cancelled, but non refundable for the paid subscription period.', 'N', '2', 'SYSTEM', 'N', 'N', 0, 20)
GO
...
DECLARE @ACCESS_ID AS INT = ( SELECT TOP 1
	ID
FROM AUTH_ACCESS
WHERE ACCESS_CD = 'STCHARLES_PERPAGE_ACCESS')
