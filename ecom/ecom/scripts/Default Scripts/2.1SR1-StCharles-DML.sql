...
VALUES ('STCHARLES', 'St. Charles County', 'Y', 'St. Charles', 'MO', 'Y', 'America/Chicago', @NODE_ID, GETDATE(), GETDATE(), 'SYSTEM', 
	'You can choose only one of the available subscriptions of a particular type for St. Charles County Record Access.', 'Y', 'Y', 'St. Charles County', 'SYSTEM', 3, 30, 30)
GO
...
INSERT INTO SITE_CONFIGURATION (SITE_ID, EMAIL_TEMPLATE_FOLDER, FROM_EMAIL_ADDRESS, PAYMENT_CONF_SUB, CHANGE_SUBSCRIPTION_SUB, CANCEL_SUBSCRIPTION_SUB, 
	...
	REMOVE_SUBSCRIPTION_TEMPLATE, CREATED_BY, ACCESS_AUTHORIZATION_SUB, ACCESS_AUTHORIZATION_TEMPLATE, PAYASUGO_PAYMENT_CONFIRMATION_SUB, PAYASUGO_PAYMENT_CONFIRMATION_TEMPLATE)
VALUES (@SITE_ID, 'RecordsManagement/stcharles/', 'noReply@amcad.com', 'Payment Confirmation - Roam St. Charles Records Management', 
	'Subscription Change Notification - Roam St. Charles Records Management', 'Cancelled Subscription Notification - Roam St. Charles Records Management', 
	'Subscription Reactivated - Roam St. Charles Records Management', 'Recurring Payment Posted Successfully - Roam St. Charles Records Management', 
	'Recurring Payment Failed - Roam St. Charles Records Management',
	...
	'recurringPaymentUnsuccessfulTemplate.stl', GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'Web Purchase Confirmation - Roam St. Charles Records Management', 
	'webPaymentConfirmation.stl', 'Subscription Removed - Roam St. Charles Records Management','removeSubscription.stl', 'SYSTEM', 
	'Access Authorization - Roam St. Charles Records Management', 'accessAuthorization.stl', 'Pay As You Go Purchase Confirmation - Roam St. Charles Records Management', 'payAsYouGoPaymentConfirmation.stl')
GO
...
VALUES (@SITE_ID, 'ecommstcharles', '047DF6A3848AB158F878321E1FA73E952BED8E3FCC1D6DF81024B364EF96DBE860B48336B54003D278118CECF6196FD8', 'amcadepay', 'Y', 'paypal', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'SYSTEM', '2.2','0.30','3.50','0.00')
GO

DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'STCHARLES')
INSERT INTO ECOMM_MERCHANTINFO (SITE_ID, USERNAME, PASSWORD, VENDORNAME, ACTIVE, PARTNER, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, IS_MICROPAYMENT_ACCOUNT, CREATED_BY, TRAN_FEE_PERCENTAGE, TRAN_FEE_FLAT, TRAN_FEE_PERCENTAGE_AMEX, TRAN_FEE_FLAT_AMEX)
VALUES (@SITE_ID, 'stcharlesmicro', 'DA68FCE9EA69E76A623CB63C151913DAF3E581129BC83C6F6E231E445C40E35DED29B9419875EB25713030138E6394F6', 'amcadepaymicro', 'Y', 'paypal', GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM', '5.0','0.05','5.0','0.05')
GO
...
VALUES (@SITE_ID, 'MAG834109743', '24F89F36C0FA68AB37A18F654C65ED0D8F66106CF6CEDFCF0A0CF3CDC208D8D5', 'AMCAD', 1, 1, 101, 'Y', GETDATE(), GETDATE(), 'SYSTEM', 'SYSTEM')
GO
...
VALUES ('STCHARLES_ADMIN', 'St. Charles County ROAM ADMIN', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'ACCESS TO THE ROAM DESKTOP.', 'N', NULL, 'SYSTEM', 'N', 'N')
...
VALUES ('STCHARLES_PERPAGE_ACCESS', 'Pay as You Go ($2.00 For First Page; $1 Per Page For Each Page After) with Image Preview (Free)', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'Access to St. Charles County Records Management. This does not include access to the images. Please choose the Premium subscription to get access to the images.', 'N', '1', 'SYSTEM', 'N')
INSERT INTO AUTH_ACCESS (ACCESS_CD, ACCESS_DESCR, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, GUEST_FLG, ACTIVE, ACCESS_FEATURES, DEFAULT_ACCESS_FLG, ACCESS_ORDER, CREATED_BY, IS_AUTHORIZATION_REQUIRED)
VALUES ('STCHARLES_ALL_ACCESS', 'Monthly Unlimited Official ($350 Every Month)', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'Access to St. Charles County Land Records along with Image access. The subscription fee is $350 per month until cancelled, but non refundable for the paid subscription period.', 'N', '2', 'SYSTEM', 'N')
GO

--- INSERT SITE ACCESS INFORMATION ---
DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'STCHARLES')
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'STCHARLES_ADMIN')
...
SET @ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'STCHARLES_PERPAGE_ACCESS')
INSERT INTO ECOMM_SITE_ACCESS (SITE_ID, ACCESS_ID, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, CREATED_BY)
VALUES (@SITE_ID, @ACCESS_ID, GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')

SET @ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'STCHARLES_ALL_ACCESS')
INSERT INTO ECOMM_SITE_ACCESS (SITE_ID, ACCESS_ID, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, CREATED_BY)
VALUES (@SITE_ID, @ACCESS_ID, GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')
GO
...
DECLARE @SITEACCESS_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE_ACCESS WHERE ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'STCHARLES_ALL_ACCESS'))
...
INSERT INTO AUTH_USERS (EMAIL_ID, PASSWORD, FIRST_NAME, LAST_NAME, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ADDRESS_1, ADDRESS_2, CITY, STATE, ZIP, PHONE, CREATED_IP, 
				...
				VALUES ('stcharledcountyadmin@roam.com', '2851FFD2E30E91904A9E86F326E442065D75DB65A028854E9A5ACEE67E53EA59', 'ROAM', 'ADMIN', GETDATE(), GETDATE(), 'ADMIN', null, null, null, null, null, null, '1.1.1.1.1', 
						'Y', 'Y', 'Y', 'Y', GETDATE(), 'RECORDSMANAGEMENT', GETDATE(), 'SYSTEM')
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'STCHARLES_ADMIN')
DECLARE @USER_ID AS INT = (SELECT TOP 1 ID FROM AUTH_USERS WHERE EMAIL_ID = 'stcharledcountyadmin@roam.com')
...
VALUES (@SITE_ID, 'St. Charles County Circuit Court', '9483 Kings Highway', 'Suite #3', 'St. Charles', 
'VA','22485','5407753322','','','OTC',GETDATE(), GETDATE(), 'SYSTEM','Y', 'SYSTEM')
GO

--- INSERT PROFIT SHARE ---
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'STCHARLES_ALL_ACCESS')
...
--- INSERT CUSTOMER BANK DETAILS FOR ACH ---
DECLARE @SITEID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'STCHARLES')
...
VALUES (@SITEID, 'St. Charles County', '101000019', '176360658', 'Commerce Bank', '1101 First Capitol Drive', 'St. Charles', 'MO', '63301', GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')
GO