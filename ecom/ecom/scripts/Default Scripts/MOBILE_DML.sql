...
VALUES ('MOBILE', 'Mobile County', 'Y', 'Mobile', 'AL', 'Y', 'America/Chicago', @NODE_ID, GETDATE(), GETDATE(), 'SYSTEM', 
	'Please create a account in the system for Mobile County Record Access.', 'Y', 'Y', 'Mobile County', 'SYSTEM', 3, 30, 30, 3)
GO

--- INSERT SITE CONFIGURATION INFORMATION ---
DECLARE @SITE_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE WHERE NAME = 'MOBILE')
...
VALUES (@SITE_ID, 'Records Management/MOBILE/', 'noReply@amcad.com', 'Payment Confirmation - Roam Mobile Records Management', 
	'Subscription Change Notification - Roam Mobile Records Management', 'Cancelled Subscription Notification - Roam Mobile Records Management', 
	'Subscription Reactivated - Roam Mobile Records Management', 'Recurring Payment Posted Successfully - Roam Mobile Records Management', 
	'Recurring Payment Failed - Roam Mobile Records Management',
	...
	'recurringPaymentUnsuccessfulTemplate.stl', GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'Web Purchase Confirmation - Roam Mobile Records Management', 
	'webPaymentConfirmation.stl', 'Subscription Removed - Roam Mobile Records Management','removeSubscription.stl', 'SYSTEM', 
...
VALUES ('MOBILE_ADMIN', 'Mobile County ROAM ADMIN', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'ACCESS TO THE ROAM DESKTOP.', 'N', NULL, 'SYSTEM', 'N')
INSERT INTO AUTH_ACCESS (ACCESS_CD, ACCESS_DESCR, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, GUEST_FLG, ACTIVE, ACCESS_FEATURES, DEFAULT_ACCESS_FLG, ACCESS_ORDER, CREATED_BY, IS_AUTHORIZATION_REQUIRED)
VALUES ('MOBILE_FREE_ACCESS', 'Mobile County Free Acces', GETDATE(), GETDATE(), 'SYSTEM', 'N', 'Y', 'Access to Mobile County Records Management.', 'Y', '1', 'SYSTEM', 'N')
GO
...
SET @ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'MOBILE_FREE_ACCESS')
INSERT INTO ECOMM_SITE_ACCESS (SITE_ID, ACCESS_ID, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ACTIVE, CREATED_BY)
VALUES (@SITE_ID, @ACCESS_ID, GETDATE(), GETDATE(), 'SYSTEM', 'Y', 'SYSTEM')
GO
...
DECLARE @SITEACCESS_ID AS INT = (SELECT TOP 1 ID FROM ECOMM_SITE_ACCESS WHERE ACCESS_ID = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'MOBILE_FREE_ACCESS'))
...
INSERT INTO ECOMM_BANK_DETAILS (SITE_ID, FROM_FNAME, FROM_LNAME, FROM_MINITIAL, FROM_ADDRLINE1, FROM_ADDRLINE2, FROM_CITY, FROM_STATE, FROM_ZIPCODE, FROM_PHONENUM,
						...
						VALUES (@SITE_ID, 'AMCAD', '', '','15867 N Mountain Road','','Broadway','VA','22815','','Capital Bank','65-334/550','055003340','111989111', 70000, 70000, 79999,
...
INSERT INTO AUTH_USERS (EMAIL_ID, PASSWORD, FIRST_NAME, LAST_NAME, DATE_TIME_CREATED, DATE_TIME_MOD, MOD_USER_ID, ADDRESS_1, ADDRESS_2, CITY, STATE, ZIP, PHONE, CREATED_IP, 
				...
				VALUES ('mobilecountyadmin@roam.com', '2851FFD2E30E91904A9E86F326E442065D75DB65A028854E9A5ACEE67E53EA59', 'ROAM', 'ADMIN', GETDATE(), GETDATE(), 'ADMIN', null, null, null, null, null, null, '1.1.1.1.1', 
						'Y', 'Y', 'Y', 'Y', GETDATE(), 'RECORDSMANAGEMENT', GETDATE(), 'SYSTEM')
DECLARE @ACCESS_ID AS INT = (SELECT TOP 1 ID FROM AUTH_ACCESS WHERE ACCESS_CD = 'MOBILE_ADMIN')
DECLARE @USER_ID AS INT = (SELECT TOP 1 ID FROM AUTH_USERS WHERE EMAIL_ID = 'mobilecountyadmin@roam.com')
...
VALUES (@SITE_ID, 'Mobile County Probate Court', 'P.O. Box 7', '', 'Mobile', 'AL','36601','2515746070','','','OTC',GETDATE(), GETDATE(), 'SYSTEM','Y', 'SYSTEM')
GO
