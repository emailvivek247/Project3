...
/**
 * A <tt>PhoneticFilter</tt> is a <tt>TokenFilter</tt> according to the
 * Lucene search-engine, which uses a {@link PhoneticEncoder}.
 * 
...
public class PhoneticFilter extends TokenFilter {
	 ...
	 * {@link TokenStream} using the given phonetic encoder. If no encoder is
	 * given, i.e. if it is <code>null</code>, no encoding takes place and
	 * the input TokenStream remains unchanged.
	 */
	public PhoneticFilter(final TokenStream in,
			final PhoneticEncoder phoneticEncoder) {
		super(in);
		input = in;
		encoder = (phoneticEncoder != null) ? new PhoneticEncoder[] { phoneticEncoder }
				: null;
	...
	public Token next() throws IOException {
		if (encoder == null) {
			...
			if (t == null)
				return null;
			return (encoder[0] != null) ? new Token(encoder[0].generateKey(t.termText()), t.startOffset(), t.endOffset(), t.type()) : t;
		} else {
			++actualIndex;
			...
			if (encoder[actualIndex] == null)
				...
				return new Token(encoder[actualIndex].toString()
						+ ":"
						+ encoder[actualIndex].generateKey(actualToken
								.termText()), actualToken.startOffset(),
						actualToken.endOffset(), actualToken.type());
		}
	}
}
