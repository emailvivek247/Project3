...
public class SearchAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            if(sc.debug) logger.info("Got config: " + (System.currentTimeMillis() - _start));

            // Get searchers from the indexNames
            sc.irs = getIndexReaderSearcher(sc.indexNames);
            if (sc.irs == null) {
                if(sc.debug) logger.warn("Can not get searcher ");
            ...
            if (!sc.dc.getIsEmptyQueryMatchAll() && U.isEmpty(q) && U.isEmpty(lq)) {
            	...
            	query = getSearchQuery(sr, q, lq, filterResult, request, sc.dc, sc.irs, getBooleanOperator(request), request.getParameter("searchable"), U.getInt(request.getParameter("randomQuerySeed"),0), sc.debug);
            }
            ...
            if (rowsToReturn <= 0) rowsToReturn = 100;
            ...
            if (query != null) {
                ...
                if (hits != null && hits.length() > 0) {
                    ...
                    if(!U.isEmpty(defaultQ)) {
                        Query defaultQuery = getSearchQuery(sr, defaultQ, lq, filterResult, request, sc.dc, sc.irs, getBooleanOperator(request), request.getParameter("searchable"), U.getInt(request.getParameter("randomQuerySeed"),0), sc.debug);
            ...
            sr.init(sc, q, lq, query, docs, defaultDocs, searchTime, total[0], offset, rowsToReturn, sortBys, filterResult, request, response);
            sr.setAttributes();
            request.setAttribute("searchResult", sr);

            //logger.debug("Set velocity variables: " + (System.currentTimeMillis() - _start));

            if(sc.debug) logger.info("Found " + total[0] + " MATCHING with \"" + q + "\" in " + searchTime + " milliseconds");

            // searcher.close();

            log(q, lq, request, sc.indexName, (sc.actualTemplateName ==null ? sc.templateName : sc.actualTemplateName),
    ...
    /**
     * indexName: Optional. If empty, all indexes are searched. If using multiple indexes, the index names are separated by comma.
     * <br/>
     * templateName: Optional. If empty, use the default template. <br/>
     *   If it's in the format of "x|y" and together with multiple index, "indexName=a,b,c", search will use the template "y" defined
     *   in index "x".
     * <br/>
     * fileName: Optional. Default is main.stl. For backward compatibility, if main.stl doesn't exists and main.vm exists, main.vm will be used.
     *   It can also be main.jsp to support jsp rendering, or any file that's rendering searchResult object.
     *   For example, tags.stl or documents.stl, for a partial page rendering. This partial rendering would be great for AJAX.
     * <br/>
     */
    public static SearchContext findSearchContext(ActionMapping mapping, HttpServletRequest request) {
        ...
        if(sc.dc==null){
            if(sc.debug) logger.info("Can not find this Index for rendering: " + sc.actualIndexName);
            return sc;
        }else{
            if(sc.debug) logger.info("search index: " + sc.indexName);
            if(sc.debug) logger.info("Using This Index to render: " + sc.actualIndexName);
        }
        ...
        if(sc.debug) logger.info("templateName:" + templateName + " templateFile:" + m_templateFile);
    ...
    protected static ArrayList<SearchSort> findSearchSorts(DatasetConfiguration dc, HttpServletRequest request) {
        ...
        if (U.isEmpty(sortBy)) return null;
        ...
        if (U.isEmpty(descendings)) return ret;
    ...
    public static Query getSearchQuery(SearchResult sr, String q, String lq, FilterResult filterResult, HttpServletRequest request, DatasetConfiguration dc, IndexReaderSearcher irs, int booleanOperator, String dynamicSearchable, int randomQuerySeed, boolean debug) {
        // parse the query
        ...
        try {
            if("Y".equalsIgnoreCase(request.getParameter("lucene"))) {
                ...
                //query = SearchQueryParser.parse(dc, analyzer, q, filteredColumns);
                ...
                if(!U.isEmpty(q)) {
                    ...
                    if(debug) logger.info("Advanced query: "+advancedQuery);
                    if(aqa.getIsAllNegative()&&advancedQuery!=null) {
                        advancedQuery = AdvancedQueryAnalysis.appendQuery(advancedQuery, new MatchAllDocsQuery(), Occur.MUST);
                    }
                }
                if(!U.isEmpty(q)) {
                    QueryTranslator translator = new QueryTranslator(dc.getColumns(), dynamicSearchable);
                    if(debug) logger.info("Start parse query: "+HTMLEntities.encode(q));
                    ...
                    if(debug) logger.info("parsed query: "+HTMLEntities.encode(myQuery.toString()));
                    ...
                    if(debug) logger.info("translated query: "+HTMLEntities.encode(query.toString()));
                ...
                if(!U.isEmpty(lq)) {
                    ...
                    if(debug) logger.info("added Lucene query: "+HTMLEntities.encode(query.toString()));
                }
            }
            if(dc.getDateWeightColumnName()!=null) {
                query = new TimeWeightQuery(query, dc, irs.getIndexReader());
                if(debug) logger.info("added time-weighted query: "+HTMLEntities.encode(query.toString()));
            }
            if(randomQuerySeed!=0) {
                query = new RandomQuery(query, randomQuerySeed);
                if(debug) logger.info("added random query: "+HTMLEntities.encode(query.toString()));
            }
        } catch (Exception ex) {
            if(debug) logger.info("Exception Occurred", ex);
            if(debug) logger.error("Cannot parse query: " + q);
        }
        //logger.debug("Parsed Query: " + (System.currentTimeMillis() - _start));
        return query;
    }

    public static Query getSearchQuery(String q, String lq, FilterResult filterResult, HttpServletRequest request, DatasetConfiguration dc, IndexReaderSearcher irs, int booleanOperator, String dynamicSearchable, int randomQuerySeed, boolean debug) {
        //this is used for cases you don't have a SearchResult available
        return getSearchQuery(null, q, lq, filterResult, request, dc, irs, booleanOperator, dynamicSearchable, randomQuerySeed, debug);
    }
    protected static void narrowBySearch(Query query, IndexReaderSearcher irs, DatasetConfiguration dc, FilterResult filterResult, ActionMessages errors, HttpServletRequest request) throws IOException {
        try{
    ...
    protected static Hits directSearch(Query query, IndexReaderSearcher irs, DatasetConfiguration dc, Hits hits, ActionMessages errors, HttpServletRequest request) throws IOException {
        try {
    ...
    protected static TopDocs topDocsSearch(Query query, IndexReaderSearcher irs, DatasetConfiguration dc, ActionMessages errors, HttpServletRequest request, int topNDocs) throws IOException {
        try {
            //search when no sortBy search
            // for debugging: dc.setDateWeightColumnName(dc.getModifiedDateColumn().getColumnName());
    ...
    protected static Hits sortBySearch(Query query, IndexReaderSearcher irs, DatasetConfiguration dc, List<SearchSort> sortBys, HttpServletRequest request, Hits hits, ActionMessages errors) throws IOException {
        try{
            return irs.getSearcher().search(getAccessFilterQuery(dc,query, request, errors), SearchSort.getLuceneSort(sortBys));
        }catch(RuntimeException re){
    ...
    protected static List<HitDocument> collectHits(DatasetConfiguration dc, Hits hits,int rowsToReturn, int offset, int[]total) throws IOException {
	...
	protected static List<HitDocument> collectHits(DatasetConfiguration paramDatasetConfiguration, TopDocs paramTopDocs,
			Searcher paramSearcher) throws CorruptIndexException, IOException {
		...
		if (paramTopDocs != null) {
			...
			for (int i = 0; i < paramTopDocs.scoreDocs.length; i++) {
				...
				localArrayList.add(new HitDocument(paramDatasetConfiguration, doc, paramTopDocs.scoreDocs[i].score, paramTopDocs.scoreDocs[i].doc));
			}
		}
		return localArrayList;
	}

    protected static Query getAccessFilterQuery(DatasetConfiguration dc, Query query, HttpServletRequest request, ActionMessages errors) {
        if(dc.getIsSecure()) {
    ...
    /*
     * If indexName=... is null, do nothing
     * If indexName is _ALL_, process for all indexes
     * If indexName has comma inside, split the indexNames and process them
     * If not anything above, process this single indexName
     */
    public static String[] getIndexNames(String indexName){
        if(U.isEmpty(indexName)) return null;
    ...
    public static IndexReaderSearcher getIndexReaderSearcher(String[] indexNames) throws Exception{
        if(indexNames == null) return null;
        ...
        IndexReaderSearcher irs = IndexReaderSearcher.getIndexReaderSearcher(irss,BufferIndexManager.getIndex(indexNames[0],false));
        return irs;
    }
    ...
    public static class SearchContext{
        ...
        /*
         * derived from "indexName" parameter, this is acutally been used to get the list of index searchers
         */
        public String[] indexNames;
        /*
         * same as "indexName" parameter, which may have several index names. It's only used for displaying messages, and to pass to the result rendering
         */
        public String indexName;
        /*
         * same as "templateName" parameter. It's only used to pass to the result rendering
         */
        public String templateName;
    }
    public static void log(String q, String lq, HttpServletRequest request, String indexName, String templateName, long visitTime, long searchingTime, long renderTime, int returnedDoc) {
        ...
        if(!U.isEmpty(q)){
            QueryLogger.log(request.getRemoteUser(), ipaddress, HTMLEntities.encode(q), indexName, templateName, visitTime, searchingTime, renderTime, returnedDoc);
        }else if(!U.isEmpty(lq)){
            QueryLogger.log(request.getRemoteUser(), ipaddress, HTMLEntities.encode(lq), indexName, templateName, visitTime, searchingTime, renderTime, returnedDoc);
        }
    }
