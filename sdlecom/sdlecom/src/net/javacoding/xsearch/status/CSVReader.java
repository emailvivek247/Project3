...
public class CSVReader {
	...
	public String [] getLine(int size) {
	    String token = null;
		String returnArray[] = new String[size];
		int index=0;
		try {
		    boolean done=false;
			while (index<size && (token = get()) != null) {
					returnArray[index]=token;
					if (index==size-1) done=true;
					index++;
			} // end of while
		    ...
		    logger.error("Not able to parse log:"+e.getMessage());
		    return null;
		}		
		return returnArray;
	...
	public String [] getLine() {
		...
		for (int ii = 0; ii < lineArray.size(); ii++) {
			returnArray[ii] = (String)lineArray.get(ii);
		} // end of for
	 ...
	 */
	private static String testSingleTokens(String[] args) {
	 ...
	 *            [0]: The name of the file.
	 */
	public static String testLines(String fileName) {
		int lineCounter = 0;
		...
		if (debugging) {
			try {
				...
				while ((loadLine = csv.getLine()) != null) {
					...
					logBuffer.append(loadLine[0]); // write first token, then
					// write DEL in loop and the
					// whole rest.
					...
					//logLine.delete(logLine.lastIndexOf(DEL),
					// logLine.length()); // is supported since JDK 1.4

					//System.out.println("#" + lineCounter +" : '" +
					// loadLine.length + "' " + logLine);
