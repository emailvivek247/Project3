...
public class SchedulerImpl implements Scheduler {
    static Logger logger = LoggerFactory.getLogger("net.javacoding.xsearch.core.task.impl.SchedulerImpl");

    protected IndexerContext ic;
    
    public class TaskContext{
        /*
         * All tasks or objects are put into queues
         * Each queue is for storing results of previous Main Query, Subsequent Query
         * Each queue also serves as inputs for the current task
         */
    ...
    public SchedulerImpl(IndexerContext ic) {
        ...
        try{
        	...
        	for(int i=0;i<taskQueueCount;i++){
        		...
        		if(i==0){
                    taskContexts[i].taskQueue = new DiskBackedQueue(ic.getDatasetConfiguration().getWorkDirectoryFile(),"result"+i,false,5);
                    taskContexts[i].maxThreadCount = 1;
        		}else if(i == taskQueueCount-1 ){
        			taskContexts[i].taskQueue = new DiskBackedQueue(ic.getDatasetConfiguration().getWorkDirectoryFile(),"result"+i,false,10);
            		...
            		int bufferSize = ic.getDatasetConfiguration().getFetcherThreadsCount()*contentQueries[i-1].getBatchSize();
            		if(bufferSize<10){
            			bufferSize = 100;
            		}
            		taskContexts[i].taskQueue = new DiskBackedQueue(ic.getDatasetConfiguration().getWorkDirectoryFile(),"result"+i,false,bufferSize);
            ...
            logger.error(NOTIFY_ADMIN, "can not create queue files under directory "+ic.getDatasetConfiguration().getWorkDirectoryFile(), ioe);
        }
    }
     ...
     * @param obj object to be scheduled
     */
	...
	public void flagStop(WorkerTask task) {
    	...
    	//but sometimes the task will flagDone() after getxxxTask(), causing deadlock waiting on the events
    ...
    public WorkerTask getWriterTask() throws TaskAssignmentException {
    	...
    	synchronized (taskContexts[i]) {
        	while(taskContexts[i].taskQueue.length()<=0 || taskContexts[i].threadCount>taskContexts[i].maxThreadCount) {
        		try {
        	        if(this.getIndexerContext().isStopping()) return null;
        	        if (allTasksDone()) {
        	        	stopPipeline();
        	        }
    ...
    private WorkerTask createBatchDetailFetcherTask(DiskBackedQueue queue, int i, int batchSize){
        synchronized(queue) {
    		//do nothing if not all documents are received or batch size is not reached
        	if(queue.length() < batchSize && !previousQueueEmpty(i)) return null;
