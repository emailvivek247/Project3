...
(function($){
	jQuery.fn.comboselect = function(settings){
		settings = jQuery.extend({
			sort: 'left',		// which sides to sort: none, left, right, or both
		...
		this.each(function(index){
			...
			combo += '</select>';
			combo += '<fieldset>';
			combo += '<input type="button" class="csup" onClick="moveToFirst(this)" value="Top" />';
			combo += '<input type="button" class="csup" onClick="moveUp(this)" value="Up" />';
			combo += '<input type="button" class="csdown" onClick="moveDown(this)" value="Down" />';
			combo += '<input type="button" class="csdown" onClick="moveToLast(this)" value="Bottom" />';
			combo += '</fieldset>';
			combo += '</fieldset>';			
			...
			theForm.submit(function(){
				// clear the original form element of all options
        ...
        this.each(function(){
            ...
            var left = $(this).parent().parent().find('select.csleft');
            var originalSelectedOpts = $(this).parent().parent().find('#'+this.id+' option:selected');
            originalSelectedOpts.each(function(){
                // select the corresponding option from the original element
                var v = $(this).attr('value');
                left.find('option[value="' + v + '"]').attr('selected','selected');
            });
            $('input.csadd').click();
